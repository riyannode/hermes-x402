...
def _resolve_symlink(p: Path) -> Path:
    ...
    prev = None
    while p.is_symlink() and p != prev:
        prev = p
        p = Path(os.readlink(str(p)))
    return p


def _parse_hermes_shim(path: Path) -> Path | None:
    """If *path* is a Bash shim of the form ``exec "/…/venv/bin/hermes" "$@"``,
    return the target path.  Returns ``None`` for non-shim files."""
    try:
    ...
    if not first.startswith("#!/"):
        return None
    # Reject non-bash/sh shebangs early
    shebang_prog = first.split(None, 1)[-1] if " " in first else ""
    _REJECT_SH = {"bash", "sh"}
    if shebang_prog in _REJECT_SH or shebang_prog.endswith(("/bash", "/sh")):
        return None
    # Match exactly: exec "<target>" "$@"
    m = re.match(r'exec\s+"([^"]+)"\s+"\$@"', lines[1].strip())
    if m:
...
def _detect_python_from_shebang(exe: Path) -> Path | None:
    """Read a Python shebang from an executable script.  Returns None if
    the shebang points to a non-Python interpreter."""
    try:
    ...
    # Split off arguments (e.g. "#!/usr/bin/env python3")
    parts = interpreter.split()
    if not parts:
        return None
    prog = parts[0]
    # Reject shell interpreters
    _REJECT = {"/bin/sh", "/bin/bash", "/usr/bin/env"}
    if prog in _REJECT or prog.endswith(("bash", "sh")) and prog.startswith("/"):
        return None
...
def _detect_python_env(
    ...
    if shim_target is None:
        # Not a shim — try reading shebang directly
        shim_target = resolved

    # Try shebang on the (possibly shim) target
...
def _run_uninstall(hermes_exe: Path, python: Path) -> dict:
    ...
    2. pip uninstall -y
    3. Verify entry-point record is absent
    4. Restart gateway (best-effort)
    """
    ...
    try:
        ...
        # 3. Verify absence
        try:
            importlib.metadata.version(_PLUGIN_NAME)
            report["errors"].append("Package still importable after uninstall")
        ...
        if r.returncode == 0:
            ...
            print(f"[uninstall] Gateway restart returned rc={r.returncode} (non-fatal)")
...
def _validate_body_file(
    ...
    if method == "POST":
        ...
        try:
            ...
            canonical = json_mod.dumps(parsed, sort_keys=True, separators=(",", ":"))
...
def run_install(
    ...
    try:
        ...
        # 3. Validate Python can import hermes_cli
    ...
    # Never print secrets
    for key in ("entity_secret", "api_key", "private_key", "otp", "password", "authorization"):
        report.pop(key, None)
...
def main() -> None:
    ...
    if args.live_test:
        if not args.service_url:
            print("[install] ❌ --live-test requires --service-url", file=sys.stderr)
            sys.exit(1)
        if not args.max_payment:
            print("[install] ❌ --live-test requires --max-payment", file=sys.stderr)
            sys.exit(1)
